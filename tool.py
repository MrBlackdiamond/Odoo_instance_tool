...
def get_user_input():
    ...
    db_user = "odoo"  # Use the existing 'odoo' user
    db_password = input("Enter the PostgreSQL password for the 'odoo' user: ")
...
def clone_odoo_source(instance_name):
    print("Cloning Odoo 16.0 source code...")
    run_command(["sudo", "git", "clone", "https://github.com/odoo/odoo.git", "-b", "16.0", "--single-branch", f"/opt/{instance_name}"])
...
port = {odoo_port}
addons_path = /opt/{instance_name}/addons,/opt/{instance_name}/custom/addons
...
User=odoo
Group=odoo
ExecStart=/opt/{instance_name}/odoo-bin -c /etc/odoo/{instance_name}.conf
Restart=on-failure
